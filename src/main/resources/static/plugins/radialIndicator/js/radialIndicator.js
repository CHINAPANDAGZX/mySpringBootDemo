    ...
    Indicator.prototype = {
        ...
        value: function (val) {
            ...
            if (indOption.displayNumber) {
                ...
                var index = dispVal.indexOf('<br>');
                if(index == -1){
                    ctx.fillText(dispVal, center, center);
                }else{
                    var texts=dispVal.split("<br>");
                    for (var k in texts) {
                        if(k == 0){
                            ctx.fillText(texts[k], center, center - (fontSize*1.5));
                        }else if(k == 1){
                            ctx.font = weight +" "+ fontSize * 2 + "px " + cFont;
                            ctx.fillText(texts[k], center, center);
                        }else if(k == 2){
                            ctx.font = weight +" "+ fontSize + "px " + cFont;
                            ctx.fillText(texts[k], center, center + (fontSize*1.5));
                        }
